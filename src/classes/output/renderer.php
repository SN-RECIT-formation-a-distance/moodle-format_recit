...
class EditingMode extends LocalRenderer{
    ...
    public function render(){
        ...
        $data->menu->addSectionUrl = "{$CFG->wwwroot}/course/changenumsections.php?courseid={$this->course->id}&insertsection=0&sesskey=$sesskey";
        $data->menu->content = '';        

        foreach ($this->sectionslist as $section) {
            $sectionId = $this->get_section_id($section);

            $data->menu->content .= $this->section_header($section, $this->course, false, 0, false, true, true);
            ...
            $data->menu->content .= "<div data-course-section-cm-list='1'>". $this->get_course_section_cm_list_editing($section)."</div>";
            $data->menu->content .= str_replace('##DATA_SECTION##', $section->section, $massaction);
            $data->menu->content .= \html_writer::end_tag('div');
            $data->menu->content .= $this->section_footer();
        }
        
        return $this->output->render_from_template('format_recit/editing_mode', $data);
    }
    ...
    public function section_title($section, $course) {
        global $CFG;
        $lastSection = $this->get_last_section();
        ...
        $sectionname .= "<a class='h3' href='$editSectionUrl'>". $this->get_section_name($section) ."</a>";
    ...
    public function section_availability($section){
        ...
        return \html_writer::div(renderer::$instance->render($widget), 'section_availability');
    }

    protected function get_course_section_cm_list_editing($section, $sectionreturn = null, $displayoptions = []) {
        $output = '';
        if (is_object($section)) {
            $section = $this->modinfo->get_section_info($section->section);
        } else {
            $section = $this->modinfo->get_section_info($section);
        }
        
        if (!empty($this->modinfo->sections[$section->section])) {
            foreach ($this->modinfo->sections[$section->section] as $modnumber) {
                $mod = $this->modinfo->cms[$modnumber];
                if ($modulehtml = $this->course_section_cm_editing($mod, $section)) {
    ...
    public function course_section_cm_editing(\cm_info $mod, $section) {
        $output = '';
        ...
        if (!empty($cmname)) {
            ...
            // Display link itself.
            $imagedata = renderer::$instance->pix_icon('monologo', '', $mod->modname, ['class' => 'activityicon']);
    ...
    public function get_move_section_select($section, $moveSectionUrl){
        ...
        $index = 0;
        $index2 = 0;
        foreach ($this->sectionslist as $sectiono) {
            // ignore section 0
        ...
        foreach ($this->sectionslist as $sectiono) {
            ...
            $sectionName = $this->get_section_name($sectiono);
