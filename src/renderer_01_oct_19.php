...
class format_treetopics_renderer extends format_section_renderer_base {
    ...
    protected function print_home($course){
        global $CFG;
        $o = '';
       ...
       /* $o .= '<iframe class="tt-home-video" width="560" height="315" src="https://www.youtube.com/embed/18VbohDa3d8" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';*/
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) 
        {
            ...
            switch($thissection->ttsectiondisplay)
            {
                case TT_DISPLAY_TABS_LEVEL_1:
                    $displayIDLev1 = $this->get_section_id($course, $thissection);
                    $outputLev1 .= html_writer::tag('button', $sectionname, array('class' => "tt-tabs tt-tabs-lev1".($firstlev1selected ? '' : ' tt-tabs-selected'), 'data' => $displayIDLev1));
                    $firstlev1selected = true;
                    
                    if(count($blockImageLev1) > 0)
                    {
                        $outputImage .= $this->print_images_block($currentLev1Section, $course, $modinfo, $blockImageLev1, false);
                    }
                ...
                case TT_DISPLAY_TABS_LEVEL_2:
                    ...
                    {
                        $countOutputLev2++;
                        $newOutputLev2 .= html_writer::tag('button', $sectionname, array('class' => "tt-tabs tt-tabs-lev2", 'data' => $this->get_section_id($course, $thissection)));
                        ...
                        $newOutputLev3 = html_writer::start_tag('div', array('id' => $this->get_section_id($course, $thissection), 'class' => self::ID_APPEND . "tabs-block-lev3 tabs-block".($firstlev3visible ? '' : ' tt-tabs-selected'), 'style' => $firstlev3visible ? 'display:none;' : ""));
                        $firstlev3visible = true;
                    }
                    break;
                case TT_DISPLAY_TABS_LEVEL_3:
                    ...
                    {
                        $countOutputLev3++;
                        $newOutputLev3 .= html_writer::tag('button', $sectionname, array('class' => "tt-tabs tt-tabs-lev3", 'data' => $this->get_section_id($course, $thissection)));
                    }
                    break;
            }
        }
    ...
    protected function print_images_block($parentsection, $course, $modinfo, $sectionarray, $startvisible = true){
        ...
        $firstdivoptions = array('id' => self::ID_APPEND . "imagebuttons", 'class' => self::ID_APPEND . "imagebuttons", 'style' => 'display: ' . ($startvisible ? 'grid' : 'none') . '; grid-template-columns: ' . $hauto . ';');
		...
		foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            $o .= html_writer::start_tag('div', array('class' => $sectionclass, 'data' => $this->get_section_id($course, $thissection),'style' => 'position:relative;text-align: center'));
            $o .= html_writer::tag('img', '', array('class' => self::ID_APPEND . 'section-image', 'src' => $url->out(false), 'alt' => $sectionname, 'style' => 'width:'.$course->ttimagewidth.'px;margin:16px;border-style:solid'));
            if($format_options['ttsectiontitle'])
                $o .= html_writer::tag('div', $sectionname, array('class' => self::ID_APPEND . 'section-title', 'style' => 'position:absolute;left:50%;top:50%;transform:translate(-50%, -50%)'));
            $o .= html_writer::end_tag('div');
		}
		$o .= html_writer::end_tag('div');
	...
	protected function get_section_id($course, $section){
		return self::ID_APPEND . str_replace(' ', '', get_section_name($course, $section));
	}
